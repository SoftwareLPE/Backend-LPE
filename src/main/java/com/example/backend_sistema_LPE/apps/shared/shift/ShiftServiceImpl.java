...
import com.example.backend_sistema_LPE.apps.trip_cascade_backend.cascada_custom.FormatTurnConfig;
import com.example.backend_sistema_LPE.apps.trip_cascade_backend.cascada_custom.FormatType;
import com.example.backend_sistema_LPE.apps.trip_cascade_backend.cascada_custom.FormatTypeTeRule;
import com.example.backend_sistema_LPE.apps.shared.plant.Plant;
import com.example.backend_sistema_LPE.apps.trip_cascade_backend.cascada_custom.FormatTurnConfigRepository;
import com.example.backend_sistema_LPE.apps.trip_cascade_backend.cascada_custom.FormatTypeTeRuleRepository;
...
public class ShiftServiceImpl implements ShiftService {
    ...
    public ShiftDTO createShift(Long plantId, CreateShiftRequestDTO request) {
        ...
        if (existingShift != null) {
            ...
            syncWialonAliases(existingShift, request.getWialonAliasNames());
    ...
    private void syncWialonAliases(Shift shift, java.util.Set<String> aliasNames) {
        java.util.Set<String> normalizedAliasNames = normalizeAliasNames(aliasNames);
        java.util.Map<String, ShiftWialonAlias> existingByNormalized = new java.util.HashMap<>();
        if (shift.getWialonAliases() != null) {
            for (ShiftWialonAlias existing : shift.getWialonAliases()) {
                existingByNormalized.put(existing.getNormalizedAliasName(), existing);
            }
        } else {
            shift.setWialonAliases(new java.util.HashSet<>());
        }

        java.util.Set<ShiftWialonAlias> updatedAliases = new java.util.HashSet<>();
        for (String normalizedAliasName : normalizedAliasNames) {
            ShiftWialonAlias alias = existingByNormalized.get(normalizedAliasName);
            if (alias == null) {
                alias = new ShiftWialonAlias();
                alias.setShift(shift);
            }
            alias.setAliasName(normalizedAliasName);
            alias.setNormalizedAliasName(normalizedAliasName);
            updatedAliases.add(alias);
        }

        shift.getWialonAliases().clear();
        shift.getWialonAliases().addAll(updatedAliases);
    }

    private java.util.Set<String> normalizeAliasNames(java.util.Set<String> aliasNames) {
        if (aliasNames == null || aliasNames.isEmpty()) {
            return java.util.Set.of();
        }
        java.util.Set<String> normalized = new java.util.LinkedHashSet<>();
        for (String aliasName : aliasNames) {
            String normalizedAlias = normalizeExternalShiftName(aliasName);
    ...
    private java.util.Set<String> extractAliasNames(Shift shift) {
        if (shift == null || shift.getWialonAliases() == null || shift.getWialonAliases().isEmpty()) {
            return java.util.Set.of();
        }
        java.util.Set<String> aliasNames = new java.util.LinkedHashSet<>();
        for (ShiftWialonAlias alias : shift.getWialonAliases()) {
            if (alias.getAliasName() != null && !alias.getAliasName().isBlank()) {
    ...
    private String normalizeExternalShiftName(String value) {
        if (value == null) {
            return "";
        }
